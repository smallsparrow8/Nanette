"""
OKINAMI Chat History Knowledge Base

Provides searchable knowledge for Nanette about the OKINAMI community history.
"""
import os
import json
...
class OKINAMIHistorySearch:
    """Searchable interface to OKINAMI chat history for Nanette."""
    ...
    def load(self, path: str):
        ...
        try:
            ...
            self._loaded = True
            print(f"Loaded OKINAMI history: {len(self.messages)} messages, {len(self.media)} media items")
        except Exception as e:
            print(f"Error loading OKINAMI history: {e}")
    ...
    def get_context_for_query(self, query: str, max_messages: int = 10) -> str:
        ...
        context_parts = [f"[Historical OKINAMI chat context for '{query}':]"]
        for msg in results:
    ...
    def get_member_history(self, username: str, limit: int = 10) -> str:
        ...
        parts = [f"[{username}'s past messages in OKINAMI chat:]"]
        for msg in messages:
...
_history_instance: Optional[OKINAMIHistorySearch] = None


def get_okinami_history() -> Optional[OKINAMIHistorySearch]:
    """Get the global OKINAMI history instance"""
    global _history_instance
    return _history_instance


def initialize_okinami_history(knowledge_base_path: str) -> bool:
    """Initialize the global OKINAMI history instance"""
    global _history_instance
    if os.path.exists(knowledge_base_path):
        _history_instance = OKINAMIHistorySearch(knowledge_base_path)
