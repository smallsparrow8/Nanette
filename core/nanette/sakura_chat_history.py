"""
Sakura Blossom Chat History Knowledge Base

Provides searchable knowledge for Nanette about the Sakura Blossom community history.
"""
import os
import json
...
class SakuraHistorySearch:
    """Searchable interface to Sakura Blossom chat history for Nanette."""
    ...
    def load(self, path: str):
        ...
        try:
            ...
            self._loaded = True
            print(f"Loaded Sakura history: {len(self.messages)} messages, {len(self.media)} media items")
        except Exception as e:
            print(f"Error loading Sakura history: {e}")
    ...
    def get_context_for_query(self, query: str, max_messages: int = 10) -> str:
        ...
        context_parts = [f"[Historical Sakura Blossom chat context for '{query}':]"]
        for msg in results:
    ...
    def get_member_history(self, username: str, limit: int = 10) -> str:
        ...
        parts = [f"[{username}'s past messages in Sakura Blossom chat:]"]
        for msg in messages:
...
_history_instance: Optional[SakuraHistorySearch] = None


def get_sakura_history() -> Optional[SakuraHistorySearch]:
    """Get the global Sakura history instance"""
    global _history_instance
    return _history_instance


def initialize_sakura_history(knowledge_base_path: str) -> bool:
    """Initialize the global Sakura history instance"""
    global _history_instance
    if os.path.exists(knowledge_base_path):
        _history_instance = SakuraHistorySearch(knowledge_base_path)
